...
class Odom_Calc:
    ...
    def ticks_cb(self, msg):
        ...
        delta_s = (r + l) / 2   # eqn 1
        delta_theta = (r - l) / 0.1   # eqn 2

        delta_x = delta_s * cos(self.theta + delta_theta / 2)   # eqn 3
        delta_y = delta_s * sin(self.theta + delta_theta / 2)   # eqn 4
        
        
        # make sure to update self.x, self.y, self.theta
        self.x = self.x + delta_x   # eqn 5
        self.y = self.y + delta_y    # eqn 6
        self.theta = self.theta + delta_theta   # eqn 7
        if self.theta > pi:
            self.theta -= 2*pi
        if self.theta < -pi:
            self.theta += 2*pi
